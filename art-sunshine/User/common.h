/**
  ...
  * @version V1.0.0
  * @date    18-August-2020
  * @brief   This file contains all the functions prototypes for whole project.
  ******************************************************************************
  * @attention
  *
...
/* Exported types ------------------------------------------------------------*/
/* 
 * enum: @system working mode 
*/
typedef enum {
	NORMAL_MODE = 0,	/* system normal mode */
	EXCEPTION_MODE,		/* system exception mode */
	SLEEP_MODE,			/* system sleep mode */
	CLEAR_MODE,			/* system clear mode */
} System_Mode;

/* 
 * enum: @crc16 check type.
*/
typedef enum {
	CRC16_IBM = 0,  /* POLY: 0x8005 INIT: 0x0000 XOROUT: 0x0000 */
	CRC16_MODBUS,	/* POLY: 0x8005 INIT: 0xFFFF XOROUT: 0x0000 */
	CRC16_USB,		/* POLY: 0x8005 INIT: 0xFFFF XOROUT: 0xFFFF */
	CRC16_MAXIM,	/* POLY: 0x8005 INIT: 0x0000 XOROUT: 0xFFFF */
}CRC16x_Type;
	
/* 
 * enum: @crc16 check type.
 */
typedef enum {R_OK = 0, R_ERROR = !R_OK}FunStatus;
typedef unsigned char  Status;		/* Function operation report */
#define PORT_BUFFER_SIZE		1024

/* 
 * struct: @uart port data struct 
 */
typedef struct {
	uint16_t length; 
	uint8_t  port; 
...
/* Exported constants --------------------------------------------------------*/
#define PDEBUG(fmt, args...)    printf(fmt, ## args)  /* system log managment */
...
/* 
 * macro: @dynamic memory allocated size define.
*/
#define SMALL_MEMSIZE 				64       /* small memory size   */
#define MEDIUM_MEMSIZE				256		 /* medium memory size  */
#define HUGE_MEMSIZE				512      /* huge memory size    */
#define LARGE_MIMSIZE				576		 /* large memory suze   */ 
#define BIGGEST_MEMSIZE				1024	 /* biggest memory size */
...
uint8_t * Xstrcat(uint8_t *strto, uint8_t *strfrom);
